...
public class CloudStreamListenerTest {
...
//            .withQueue("test-queue") // Deprecated
        ...
        await().atMost(5, SECONDS).untilAsserted(() -> {
            try(var connection = factory.newConnection()) {
                assertThat(connection.createChannel()
                        .queueDeclarePassive("cloud-queue")
                        .getQueue())
                        .isNotNull();
            }
        });
    ...
    void testVerifyExchangeExists() {
        ...
        await().atMost(5, SECONDS).untilAsserted(() -> {
            try(var connection = factory.newConnection()) {
                assertThat(connection.createChannel()
                        .exchangeDeclarePassive("cloud-exchange"))
                        .isNotNull();
            }
        });
    }
    ...
    void testReceiveMessageDirectlyToQueue(CapturedOutput output) {
        String message = "test-cloud-something";
        String testQueue = "cloud-queue";
    ...
    void testReceiveMessageWhenPublishToExchange(CapturedOutput output) {
        String message = "test-cloud-something";
        String exchange = "cloud-exchange";
        String routing = "a.b.c";
